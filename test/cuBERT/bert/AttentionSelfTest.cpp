...
TEST_F(AttentionSelfTest, compute) {
    float query_kernel[] = {0, 1, 2, 3, 4, 5, 6, 7, 8};
    float query_bias[] = {-3, -2, -1};

    float key_kernel[] = {8, 7, 6, 5, 4, 3, 2, 1, 0};
    float key_bias[] = {0, 1, 2};

    float value_kernel[] = {-1, -1, -1, 2, 2, 2, 1, 1, 1};
    float value_bias[] = {3, 3, 3};
    ...
    cudaMalloc(&tensor_gpu, sizeof(float) * 30);
    cudaMalloc(&mask_gpu, sizeof(float) * 20);
    cudaMalloc(&out_gpu, sizeof(float) * 30);

    cudaMemcpy(tensor_gpu, tensor, sizeof(float) * 30, cudaMemcpyHostToDevice);
    ...
    EXPECT_FLOAT_EQ(out[0], 13);
    EXPECT_FLOAT_EQ(out[1], 13);
    EXPECT_FLOAT_EQ(out[2], 13);
    EXPECT_FLOAT_EQ(out[3], 13);
    EXPECT_FLOAT_EQ(out[4], 13);
    EXPECT_FLOAT_EQ(out[5], 13);
    EXPECT_FLOAT_EQ(out[6], 25);
    EXPECT_FLOAT_EQ(out[7], 25);
    EXPECT_FLOAT_EQ(out[8], 25);
    EXPECT_FLOAT_EQ(out[9], 25);
    EXPECT_FLOAT_EQ(out[10], 25);
    EXPECT_FLOAT_EQ(out[11], 25);

    EXPECT_FLOAT_EQ(out[12], 37);
    EXPECT_FLOAT_EQ(out[13], 37);
    EXPECT_FLOAT_EQ(out[14], 37);
    EXPECT_FLOAT_EQ(out[15], 37);
    EXPECT_FLOAT_EQ(out[16], 37);
    EXPECT_FLOAT_EQ(out[17], 37);
    EXPECT_FLOAT_EQ(out[18], 49);
    EXPECT_FLOAT_EQ(out[19], 49);
    EXPECT_FLOAT_EQ(out[20], 49);
    EXPECT_FLOAT_EQ(out[21], 49);
    EXPECT_FLOAT_EQ(out[22], 49);
    EXPECT_FLOAT_EQ(out[23], 49);
    EXPECT_FLOAT_EQ(out[24], 61);
    EXPECT_FLOAT_EQ(out[25], 61);
    EXPECT_FLOAT_EQ(out[26], 61);
    EXPECT_FLOAT_EQ(out[27], 61);
    EXPECT_FLOAT_EQ(out[28], 61);
    EXPECT_FLOAT_EQ(out[29], 61);
}

TEST_F(AttentionSelfTest, compute_complex) {
